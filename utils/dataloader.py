...
class TestDataset_Batch(data.Dataset):
    def __init__(self, image_root, trainsize):
        ...
        self.images = [
            image_root + f for f in image_root_dir_list if f.endswith(".jpg")
        ]
...
def get_loader_testbatch(
    image_root, batchsize, trainsize, shuffle=False, num_workers=8, pin_memory=True
):
